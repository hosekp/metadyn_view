...
$.extend(view.axi, {
  ...
  drawSvgAxisY:function (width,height) {
    ...
    for (i = 0; i < range.length; i += 1) {
      ...
      pathPath+=' M '+(width-1)+' '+pos+' l 0 -4';
      var tickText=document.createElementNS(this.svgNamespace,"text");
      tickText.textContent=this.toPrecision(range[i], dec);
      tickText.setAttribute("x",(width-30).toString());
    ...
    axiLabel.setAttribute("x",(-(height+text.length*this.letterwidth)/2).toString());
  ...
  drawSvgAxisZ:function (noY,width,height) {
    ...
    if(noY){
      ...
      pathPath='M 1 '+margin+' l 1 '+barLen
      +' l '+barwid+' 0 l 0 -'+barLen+' l -'+barwid+' 0';
      // ctx.moveTo(1,margin);
      ...
      // var barImage = document.createElementNS(this.svgNamespace,'image');
      ...
      axiLabel.setAttribute("transform","rotate(-90)");
      axiLabel.setAttribute("y",(width-5).toString());
      // axiLabel.setAttribute("y",(-(height+text.length*this.letterwidth)/2).toString());
    }
    // axiLabel.textContent=text;
    axiLabel.setAttribute("x",(-(height+text.length*this.letterwidth)/2).toString());
    textGroup.appendChild(axiLabel);

...
class MyTXT {
	...
	public $delimiter;
	public $rows;
	function MyTXT($filepath="__default__", $text_delimiter=":|:",$column_heads=NULL) {
		// read file into memory if needed
                $this->column_heads=$column_heads;
		...
		if ($filepath == "__default__") {
			...
			if (file_exists($filepath)) {
				...
				if ($fh = fopen($filepath, 'w')) {
                                    if($column_heads===NULL){
					fwrite($fh, ".");
                                    }else{
                                        fwrite($fh, implode($this->delimiter, $this->column_heads));
                                    }
					fclose($fh);
				}
				else {
